...
private:
  ...
  struct HashMapValue {
    ...
    HashMapValue(TValue && value, ListNode * node)
      : m_value(std::move(value)), m_listNode(node)
    {}
...
public:
  ...
  bool insert(TKey && key, ConstAccessor & ac, TValue && value);
...
private:
  void post_insert(ListNode * node);
...
insert(const TKey& key, const TValue& value) {
  ...
  if (!m_map.insert(hashAccessor, std::move(hashMapValue)))
  {
    delete node;
    return false;
  }
...
template <class TKey, class TValue, class THash>
bool ConcurrentLRUCache<TKey, TValue, THash>::
insert(TKey && key, ConstAccessor & ca, TValue && value)
{
  // Insert into the CHM
  ListNode* node = new ListNode(key);
  HashMapValuePair hashMapValue(std::move(key), HashMapValue(std::move(value), node));
  if (!m_map.insert(ca.m_hashAccessor, std::move(hashMapValue)))
  {
    delete node;
    return false;
  }
