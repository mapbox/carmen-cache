...
RocksDBCache::MessagesT const & RocksDBCache::fetch_messages(
    const std::string& phrase_ref, PrefixMatch match_prefixes, CacheT::ConstAccessor & ca)
{
    ...
    if (m_cache->find(ca, phrase))
      return *ca;

//    mbsc::Log(mbsc::Level::INFO) << "RocksDBCache::fetch_messages = "
//                                 << this << "; "
//                                 << phrase_ref << "; "
//                                 << match_prefixes;
    MessagesT messages;
    ...
    for (rit->Seek(phrase); rit->Valid(); rit->Next())
    {
        ...
        messages.emplace_back(rit->value().ToString(), extract_langfield(key));
    }

    m_cache->insert(std::move(phrase), ca, std::move(messages));
    return *ca;
}
...
{
    CacheT::ConstAccessor ca;
    MessagesT const & messages = fetch_messages(phrase_ref, match_prefixes, ca);
    ...
    for (auto const & message : messages) {
        ...
        bool const matches_language = static_cast<bool>(std::get<1>(message) & langfield);
...
intarray RocksDBCache::__getmatchingBboxFiltered(const std::string& phrase_ref, PrefixMatch match_prefixes, langfield_type langfield, size_t max_results, const uint64_t box[4])
{
    ...
    for (auto const & message : messages)
    {
        ...
        decodeAndBboxFilter(std::get<0>(message), array, boost, box);
    }
...
RocksDBCache::RocksDBCache(const std::string& filename) {
    ...
    m_cache = std::make_shared<CacheT>(1024);
}
